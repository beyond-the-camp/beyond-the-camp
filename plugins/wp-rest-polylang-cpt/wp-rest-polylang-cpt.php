<?php
/**
 * Plugin Name: WP REST - Polylang ("Beyond the Camp" Edition)
 * Description: Polylang integration for the WP REST API with customisations for Beyond the Camp
...
/**
 * MODIFICATIONS FROM ORIGINAL BELOW: 
 * 
 * Add support for specific Custom Page Types.
 */

function WP_REST_polylang_prepare_term($data, $term, $context) {
	$ID = $term->term_id;
	$locale = pll_get_term_language($ID, 'locale');
	...
	$translations = array_reduce($translations, function ($carry, $translation) {
		$item = array(
			'locale' => pll_get_term_language($translation, 'locale'),
			'id' => $translation
		);
...
add_filter('rest_prepare_project', 'WP_REST_polylang_prepare_post', 10, 3);

add_filter('rest_prepare_category', 'WP_REST_polylang_prepare_term', 10, 3);
